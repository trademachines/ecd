...
import { EcdClient } from './client';

const client = new EcdClient(process.cwd());
...
const callApi = (method, cluster, service) => {
  ...
  client.region       = program.region;
  client.functionName = program.function;

  return client.call(method, cluster, service, configFile, varFile, variables)
    .then((res) => {
